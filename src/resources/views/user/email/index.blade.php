...
<section class="mt-3">
    <div class="container-fluid p-0">
	    <div class="row">
	                    ...
	                    <form action="{{route('user.manage.email.search',$scope ?? str_replace('user.manage.email.', '', request()->route()->getName()))}}" method="GET">
	                        <div class="row align-items-center">
	                            <div class="col-lg-5">
	                                ...
	                                <input type="text" autocomplete="off" name="search" value="" placeholder="{{translate('Search with email')}}" class="form-control" id="search" value="{{@$search}}">
	                            </div>
	                            <div class="col-lg-5">
			...
			@php
			  $pendingLog = $emailLogs->where('status',1)->count();
		    @endphp
		                    ...
		                    @forelse($emailLogs as $emailLog)
			                    <tr class="@if($loop->even)@endif">
				                    ...
				                    <td data-label={{ translate('Action')}}>
			                    		<a class="btn--primary text--light" href="{{route('user.manage.email.view',$emailLog->id)}}" target="_blank"
