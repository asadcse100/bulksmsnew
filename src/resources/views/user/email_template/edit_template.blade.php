...
<section class="mt-3 rounded_box">
	...
	<div class="container-fluid p-0 pb-2">
		<div class="row d-flex align--center rounded">
			<div class="col-xl-12">
				<div class="col-xl">
					<form action="{{route('user.template.email.update')}}" method="POST" enctype="multipart/form-data">
						@csrf
                        ...
                        <div class="row g-2 mt-2">
                            <div class="col-md">
                                    ...
                                    <button type="submit" id="save-button"  class=" @if($template->provider == 1)
                                        d-none
                                     @endif   btn btn-primary">
                                        {{translate('Save')}}
                                    </button>
        ...
        function loadTemplate(templateId = null) {
            ...
            $.post(endpoint, payload).done(function (data) {
                ...
                window.BeePlugin.create(token, config, function (instance) {
                    bee = instance;
                    $.get(
                        `${baseUrl}/user/email/templates/edit/json/${templateId}`,
                        function (template) {
                            bee.start(template);
                        }
                    );
