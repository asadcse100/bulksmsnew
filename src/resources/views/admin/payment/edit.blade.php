			...
			<div class="col-xl-12">
				<div class="table_heading d-flex align--center justify--between">
                    <nav  aria-label="breadcrumb">
					  	<ol class="breadcrumb">
					    	<li class="breadcrumb-item"><a href="{{route('admin.payment.method.index')}}" {{ translate('Payment Method')}}</a></li>
					    	<li class="breadcrumb-item" aria-current="page"> {{ucfirst($paymentMethod->name)}}</li>
					  	</ol>
					</nav>
                    <a href="{{route('admin.payment.method.index')}}" class="btn--dark text--light border-0 px-3 py-1 rounded ms-3"><i class="las la-angle-double-left"></i>  {{ translate('Go Back')}}</a>
                </div>
				<div class="card">
