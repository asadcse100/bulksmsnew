					    		...
					    		<div class="row">
									...
									<div class="col-md-3 mb-2">
					            		...
					            		<div class="input-group input-group-merge">
								             <input class="form-control" type="text" name="name" placeholder="{{translate('Enter Name')}}" value="{{$campaign->name}}">
                                        </div>
					          		</div>
                                      ...
                                      <div class="col-md-3 mb-2">
					            		...
					            		<div class="input-group input-group-merge">
								            <select class="form-control " name="status" id="">
                                                ...
                                                <option {{$campaign->status == "Active" ? "selected" :""}} value="Active">
                                                    {{translate("Active")}}
                                                </option>
                                                <option {{$campaign->status == "DeActive" ? "selected" :""}} value="DeActive">
                                                    {{translate("DeActive")}}
                                                </option>
									...
									<div class="row">
										<div class="col-12">
											...
											<div class="input-group">
												<textarea  class="form-control" name="message" id="message" rows="2"> {{$campaign->body}}  </textarea>
											</div>
								...
								<div class="card-body">
									<div class="row">
											...
											<div class="mb-3">
												...
												<div class="input-group input-group-merge">
														<div class="form-check form-check-inline">
															<div class="form-check form-check-inline">
															<input class="form-check-input" {{$campaign->sms_type == "plain" ?'checked' :"" }}  type="radio" name="smsType" id="smsTypeText" value="plain" checked="">
															<label class="form-check-label" for="smsTypeText">{{ translate('Text')}}</label>
														</div>

														<div class="form-check form-check-inline">
															<input {{$campaign->sms_type == "unicode" ?'checked' :"" }}   class="form-check-input" type="radio" name="smsType" id="smsTypeUnicode" value="unicode">
										...
										<div class="input-group input-group-merge speech-to-text" id="messageBox">
												<textarea class="form-control" name="message" id="smsmessage" placeholder="{{ translate('Enter SMS Content &  For Mention Name Use ')}}@php echo "{{". 'name' ."}}"  @endphp" aria-describedby="text-to-speech-icon"> {{$campaign->body}}  </textarea>
				          		...
				          		<div class="row">
					          		<div class="col-md-4 ">
                                        <label for="schedule_date" class="form-label">
				                       	...
				                       	<input type="datetime-local" value= "{{$campaign->schedule_time}}" name="schedule_date" id="schedule_date" class="form-control" required="">
                                    </div>
                                    <div class="col-md-4">
                                        <label for="repeat" class="form-label">
                                            {{translate('Repeat Every')}}   <sup class="text-danger">*</sup>
                                        </label>
                                        <input type="number" required id="repeatNumber" class="form-control" value="{{$campaign->schedule->repeat_number}}" name="repeat_number"
                                            id="repeat">
                                    </div>
                                    <div class="col-md-4">
...
<script>
	(function($){
		...
		function  appendTemplate(){
			$.ajax({
				method:"GET",
				url:"{{ route('admin.template.email.list') }}",
				dataType:'json'
			}).then(response=>{
