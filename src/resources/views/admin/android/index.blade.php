	    ...
	    <div class="row">
	 		<div class="col-lg-12">
	            <div class="card mb-4">
		                    ...
		                    @forelse($androids as $android)
			                    <tr class="@if($loop->even)@endif">
				                    ...
				                    <td data-label="{{ translate('list')}}">
				                    	<a href="{{route('admin.sms.gateway.android.sim.index', $android->id)}}" class="s_btn--primary text--light">{{ translate('View All') }}</a>
				                    </td>

				                    <td data-label={{ translate('Action') }}>
			                    		<a class="btn--primary text--light android" data-bs-toggle="modal" data-bs-target="#updateandroid" href="javascript:void(0)"
			                    			data-id="{{$android->id}}"
			                    			data-name="{{$android->name}}"
			                    			data-password="{{$android->show_password}}"
			                    			data-status="{{$android->status}}"><i class="las la-pen"></i></a>
			                    		<a class="btn--danger text--light delete" data-bs-toggle="modal" data-bs-target="#deleteandroidApi" href="javascript:void(0)" data-id="{{$android->id}}"><i class="las la-trash"></i></a>
				                    </td>
			                    </tr>
			                @empty
			                	<tr>
	                ...
	                <div class="m-3">
	                	{{$androids->appends(request()->all())->links()}}
					</div>
	            </div>
	        </div>
	    </div>
