			...
			<div class="col-xl-12">
				<div class="col-xl">
					<form action="{{route('admin.whatsapp.store')}}" method="POST" enctype="multipart/form-data">
				          		...
				          		<div class="row">
					          		<div class="md-12">
					            		...
					            		<div class="input-group input-group-merge speech-to-text" id="messageBox">
										  	<textarea class="form-control" name="message" id="message" placeholder="{{ translate('Enter SMS Content &  For Mention Name Use ')}}@php echo "{{". 'name' ."}}"  @endphp" aria-describedby="text-to-speech-icon">{{session()->get('old_message')}}</textarea>
					    ...
					    <button type="submit" class="btn btn-primary me-sm-3 me-1">{{ translate('Submit') }}</button>
				    </form>
				</div>
			</div>
		</div>
